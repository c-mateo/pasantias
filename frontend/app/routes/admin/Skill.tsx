...
export async function clientLoader({ params }: Route.ClientLoaderArgs) {
  ...
  const res = await api.get(`/skills/${params.skillId}`).json<AdminSkillDetailsResponse>();
...
export default function Skill({ loaderData }: Route.ComponentProps) {
      ...
      action: async () => {
        try {
          ...
          toastHelper.info({
            ...
            description: isExisting ? "Actualizando la skill en el servidor..." : "Creando la skill en el servidor...",
            promise: opPromise.then(),
          });

          try {
            await opPromise;
            toastHelper.success({ title: isExisting ? "Skill actualizada" : "Skill creada" });
            setModal({ ...modal, isOpen: false });
            navigate("/admin/skills");
          } catch (err) {
            // Match Carrera.tsx: parse API error body and handle known validation types
            const body = await (err as WretchError).response.json().catch(() => null);
            const apiError = body as ApiError | null;
            if (apiError?.type === "already-exists") {
              setErrors({ name: "Ya existe una skill con ese nombre" });
              toastHelper.error({ title: "Conflicto", description: "Ya existe una skill con ese nombre" });
              return;
            }

            console.error(err);
            toastHelper.error({ title: "Error", description: "No se pudieron guardar los cambios." });
            return;
          }
        } catch (err) {
          console.error(err);
  ...
  const del = () => {
    setModal({ isOpen: true, message: "¿Está seguro de que desea eliminar esta skill?", action: async () => {
      try {
        ...
        console.error(err);
        toastHelper.error({ title: "Error", description: "No se pudo eliminar la skill." });
      }
    }});
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-6 gap-6">
            <div className="lg:col-span-4">
              <article className="bg-white rounded shadow">
                <div className="p-4">
                  <div className="mb-4">
                    <a href="/admin/skills" className="text-sm text-blue-600">← Volver a Skills</a>
                  </div>
            ...
            <aside className="lg:col-span-2">
              <div className="bg-white rounded shadow p-4 space-y-4">
                ...
                <div className="flex flex-col gap-3">
                  <Button color="primary" className="w-full" radius="md" onPress={save}>{item.id ? "Guardar Cambios" : "Crear Skill"}</Button>
                  {item.id !== 0 && <Button color="danger" className="w-full" radius="md" onPress={del}>Eliminar Skill</Button>}
                </div>
              </div>
            </aside>
          </div>
