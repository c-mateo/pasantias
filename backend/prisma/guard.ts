...
type Model = PublicKeys<PrismaClient> & string;
...
export async function guard<T>(
  ...
  // const model = prisma[guardSetup.model];
  ...
  try {
    ...
    if (error instanceof PrismaClientKnownRequestError) {
      ...
      throw apiErrors.internalError(
        `db_${error.code}_${Date.now().toString(36)}`,
...
export type ErrorStrategy = (error: PrismaClientKnownRequestError, model: any, args: any) => Promise<void> | void;
...
export async function executeGuarded<T>(
  ...
  try {
    ...
    if (error instanceof PrismaClientKnownRequestError) {
      ...
      throw apiErrors.internalError(`db_${error.code}`, 'admin@unraf.edu.ar');
    }
    throw error;
  }
}